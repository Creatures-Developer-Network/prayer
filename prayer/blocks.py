...
class Block:
    ...
    def _set_block_data(self,data):
        ...
        # the following 128 Byte, contain the Name of the Block in latin-1 padded with 'NUL' '\0'
        ...
        if int.from_bytes(data[140:144], byteorder='little') == 1 and data_length != uncompressed_data_length:
            self.compressed = True
        else:
            self.compressed = False
        # The Compressed and decompressed Data can each be found at offset 144 + the Length of the "self.data_length" Variable
...
class TagBlock(Block):
    ...
    def data(self,data):
        ...
        self._get_named_integer_variables(data=data[4:], count=self.number_of_integer_variables)
        # Strings
        self.number_of_string_varaibles = int.from_bytes(self.str_data[:4], byteorder='little')
        self._get_named_string_variables(data=self.str_data[4:], count=self.number_of_string_varaibles)
