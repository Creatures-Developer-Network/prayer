...
class TagBlock(Block):
    ...
    def _get_named_string_variables(self, data, count):
        ...
        if count != 0:
            ...
            value_length = int.from_bytes(
                data[4 + key_length:8 + key_length], byteorder='little')
            value = data[
                8 + key_length:8 + key_length + value_length].decode('latin-1')
            self.named_variables.append((key, value))
    ...
    def data(self):
        ...
        if True:
            ...
            for variable in ints:
                tmp_ints += (
                    len(bytes(variable[0], encoding='latin-1')).to_bytes(length=4, byteorder='little'))
                tmp_ints += bytes(variable[0], encoding='latin-1')
                tmp_ints += variable[1].to_bytes(length=4, byteorder='little')
            tmp_strings = bytes(
                len(strings).to_bytes(length=4, byteorder='little'))
            for variable in strings:
                tmp_strings += (
                    len(bytes(variable[0], encoding='latin-1')).to_bytes(length=4, byteorder='little'))
                tmp_strings += bytes(variable[0], encoding='latin-1')
                tmp_strings += (
                    len(bytes(variable[1], encoding='latin-1')).to_bytes(length=4, byteorder='little'))
    ...
    def data(self, data):
        ...
        # Integers
        self.number_of_integer_variables = int.from_bytes(
